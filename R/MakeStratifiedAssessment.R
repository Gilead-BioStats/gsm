function (lWorkflow, lData, lMapping, bQuiet = TRUE) {
    stopifnot(`[ lWorkflow ] must be a list.` = is.list(lWorkflow), 
        `[ lWorkflow ] requires a [ group ] property.` = "group" %in% 
            names(lWorkflow), `[ lWorkflow$group ] must be a list.` = is.list(lWorkflow$group), 
        `[ lWorkflow$group ] requires properties of [ domain ] and [ columnParam ].` = all(c("domain", 
            "columnParam") %in% names(lWorkflow$group)), `[ lMapping ] must be a list.` = is.list(lMapping), 
    ...
    stratifiedWorkflows <- strata %>% purrr::imap(function(stratum, 
        ...
        lStrata <- list(list(name = "FilterData", inputs = domainName, 
            output = domainName, params = list(strCol = columnName, 
                anyVal = stratum)))
        workflow$steps <- c(lStrata, lWorkflow$steps)
        workflow
    })
    names(stratifiedWorkflows) <- purrr::map_chr(stratifiedWorkflows, 
        ~.x$name)
    if (!bQuiet) {
