...
#' possible outliers. The default in `stats::fisher.test` uses two-sided test (equivalent to testing
#' the null of OR=1) and not to compute p-values by Monte Carlo simulation unless `simulate.p.value = TRUE` is
#' specified.Sites with p-values less than 0.05 from the Fisher’s exact test analysis are flagged by default.
