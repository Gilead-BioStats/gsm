function(
    lAssessments = Study_Assess(), dfSite = Site_Map_Raw(),
  ...
  if (!is.null(dfSite)) {
    overview_table <- overview_table %>%
      ...
      left_join(dfSite %>% select("siteid",
        Country = "country",
        Status = "status"
      ), by = c(Site = "siteid")) %>%
      select("Site", "Country", "Status", everything())
    overview_table <- drop_column_with_several_na(
      overview_table,
      "Country"
    )
    overview_table <- drop_column_with_several_na(
    ...
    site_status_tooltip_hover_info <- dfSite %>%
      ...
      purrr::imap(function(site_data,
        ...
        tooltip_site_data <- Filter(length, site_data[names(site_data) %in%
          site_data_variables_to_pull]) %>%
          bind_rows() %>%
          mutate(across(everything(), ~ as.character(.))) %>%
          rename(any_of(setNames(
            site_data_variables_to_pull,
            site_data_variables_to_rename
          ))) %>%
  ...
  hovertext_lookup <- tibble::enframe(abbreviation_lookup) %>%
    mutate(name = paste0(.data$name, "_hovertext")) %>%
    tibble::deframe()
  ...
  overview_table <- overview_table %>%
    ...
    arrange(.data$Site) %>%
    left_join(reference_table, by = "Site")
  if (!is.null(dfSite)) {
  ...
  if (bInteractive) {
    ...
    overview_table <- overview_table %>% mutate(across(
      names(abbreviation_lookup),
      ~ purrr::imap(.x, function(value, index) {
        kri_directionality_logo(value, title = overview_table[[paste0(
          cur_column(),
          "_hovertext"
        )]][[index]])
      })
    ...
    if (strReportType %in% c("country", "qtl", "QTL")) {
      ...
      lengthMenuOptions <- list(c(
        end_of_red_kris, end_of_red_and_amber_kris,
        -1
      ), c("Red", "Red & Amber", "All"))
    }
    overview_table <- overview_table %>% DT::datatable(
      class = "compact tbl-rbqm-study-overview",
      rownames = FALSE, escape = FALSE, caption = HTML(overview_table_flagged_caption),
      options = list(
        language = list(lengthMenu = paste0(if (strReportType ==
          "site") {
          "Sites Containing _MENU_ "
        } else if (strReportType == "country") {
          "View _MENU_  Countries"
        }, table_dropdown_label)), columnDefs = list(list(
          className = "dt-center",
        ...
        pageLength = ifelse(strReportType == "site",
          end_of_red_kris, nrow(overview_table)
