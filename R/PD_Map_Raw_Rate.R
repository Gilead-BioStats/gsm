function (dfs = list(dfSUBJ = clindata::rawplus_dm, dfPD = clindata::ctms_protdev), 
    ...
    checks <- gsm::CheckInputs(context = "PD_Map_Raw_Rate", dfs = dfs, 
        bQuiet = bQuiet, mapping = lMapping)
    if (checks$status) {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn PD_Map_Raw_Rate}")
        ...
        dfInput <- dfPD_mapped %>% group_by(.data$SubjectID) %>% 
            ...
            mutate(Rate = .data$Count/.data$Exposure) %>% select(any_of(names(dfSUBJ_mapped)), 
            "Count", "Exposure", "Rate") %>% arrange(.data$SubjectID)
        if (!bQuiet) 
            cli::cli_alert_success("{.fn PD_Map_Raw_Rate} returned output with {nrow(dfInput)} rows.")
    }
    else {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn PD_Map_Raw_Rate} did not run because of failed check.")
        dfInput <- NULL
    }
    if (bReturnChecks) {
