function (dfs = list(dfSUBJ = clindata::rawplus_dm, dfIE = clindata::rawplus_ie), 
    ...
    checks <- gsm::CheckInputs(context = "IE_Map_Raw", dfs = dfs, 
        bQuiet = bQuiet, mapping = lMapping)
    if (checks$status) {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn IE_Map_Raw}")
        dfIE_mapped <- dfs$dfIE %>% select(SubjectID = lMapping[["dfIE"]][["strIDCol"]], 
        ...
        dfInput <- dfIE_mapped %>% mutate(Expected = ifelse(.data$Category == 
            lMapping$dfIE$strCategoryVal[1], lMapping$dfIE$strResultVal[1], 
            lMapping$dfIE$strResultVal[2]), Valid = .data$Result == 
            .data$Expected, Invalid = .data$Result != .data$Expected, 
            Missing = !(.data$Result %in% lMapping$dfIE$strResultVal)) %>% 
            group_by(.data$SubjectID) %>% summarise(Total = n(), 
            Valid = sum(.data$Valid), Invalid = sum(.data$Invalid), 
            Missing = sum(.data$Missing)) %>% mutate(Count = .data$Invalid + 
            .data$Missing) %>% ungroup() %>% select("SubjectID", 
            "Count") %>% gsm::MergeSubjects(dfSUBJ_mapped, vFillZero = "Count", 
            bQuiet = bQuiet) %>% select(any_of(names(dfSUBJ_mapped)), 
            "Count") %>% arrange(.data$SubjectID)
        if (!bQuiet) 
