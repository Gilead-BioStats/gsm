function(
    dfMetaWorkflow = gsm::meta_workflow, strWorkflowIDs = NULL,
    lStatus = NULL) {
  if (length(lStatus) != 0) {
  ...
  workflows <- dfMetaWorkflow %>%
    filter(.data$workflowid %in%
      combo_strWorkflowIDs) %>%
    rename_with(~ .x %>%
      gsub("_|(?=id)",
        " ", .,
        perl = TRUE
  ...
  workflows %>%
    ...
    DT::datatable(class = "compact", options = list(
