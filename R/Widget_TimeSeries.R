function(
    results, workflow, parameters, selectedGroupIDs = NULL,
  ...
  htmlwidgets::createWidget(
    name = "Widget_TimeSeries", x,
    width = width, height = height, package = "gsm", elementId = elementId
  ) %>%
    htmlwidgets::prependContent(htmltools::tags$div(
      class = "select-group-container",
      htmltools::tags$label(siteSelectLabelValue), htmltools::tags$select(
        class = "site-select--time-series",
        id = glue::glue("site-select--time-series_{unique(workflow$workflowid)}"),
        purrr::map(c("None", uniqueSiteSelections), ~ htmltools::HTML(paste0(
          "<option value='",
