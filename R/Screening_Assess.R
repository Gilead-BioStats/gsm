function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "Screening_Assess.yaml", 
    ...
    lChecks <- CheckInputs(context = "Screening_Assess", dfs = list(dfInput = dfInput), 
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn Screening_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn Screening_Assess}")
        if (!bQuiet) 
        ...
        if (strMethod != "QTL") {
            ...
            dfConfig <- MakeDfConfig(strMethod = strMethod, strGroup = strGroup, 
                strAbbreviation = "SF", strMetric = "Screen Failure Rate", 
                strNumerator = "Screen Failures", strDenominator = "Screened Subjects", 
                vThreshold = vThreshold)
            if (strMethod != "Identity") {
                ...
                lCharts$scatterJS <- gsm::Widget_ScatterPlot(results = lData$dfSummary, 
                  workflow = dfConfig, bounds = bounds, elementId = "screeningAssessScatter", 
            ...
            lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
                workflow = dfConfig, yaxis = "metric", elementId = "screeningAssessMetric", 
                siteSelectLabelValue = strGroup)
            lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
                workflow = dfConfig, yaxis = "score", elementId = "screeningAssessScore", 
                siteSelectLabelValue = strGroup)
            if (!bQuiet) 
