function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "PD_Assess_Binary.yaml", 
    ...
    lChecks <- gsm::CheckInputs(context = "PD_Assess_Binary", 
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn PD_Assess_Binary} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn PD_Assess_Binary}")
        if (!bQuiet) 
