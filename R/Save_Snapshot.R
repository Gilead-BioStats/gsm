function (lSnapshot, cPath, bCreateDefaultFolder = FALSE, bQuiet = TRUE) {
    stopifnot(`[ cPath ] does not exist.` = dir.exists(cPath), 
    ...
    purrr::iwalk(lSnapshot$lSnapshot, ~utils::write.csv(.x, file = paste0(full_path, 
        "/", .y, ".csv"), row.names = FALSE))
    if (!bQuiet) 
        cli::cli_alert_info("Saving snapshot object as { .file snapshot.rds } to { .file { cPath } }")
    saveRDS(lSnapshot, file = paste0(full_path, "/snapshot.rds"), 
        compress = TRUE)
}
