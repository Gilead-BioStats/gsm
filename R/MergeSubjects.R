function (dfDomain, dfSUBJ, strIDCol = "SubjectID", vFillZero = NULL, 
    vRemoval = NULL, bQuiet = TRUE) {
    if (!bQuiet) 
        cli_alert_info("Intializing merge of domain and subject data")
    is_domain_valid <- gsm::is_mapping_valid(df = dfDomain, mapping = list(strIDCol = strIDCol), 
        spec = list(vUniqueCols = "strIDCol", vRequired = "strIDCol"), 
        bQuiet = bQuiet)
    is_subjects_valid <- gsm::is_mapping_valid(df = dfSUBJ, mapping = list(strIDCol = strIDCol), 
        spec = list(vUniqueCols = "strIDCol", vRequired = "strIDCol"), 
        bQuiet = bQuiet)
    stopifnot(`Errors found in dfDomain` = is_domain_valid$status, 
    ...
    if (length(domain_only_ids > 0)) {
        if (!bQuiet) {
            cli::cli_alert_warning(paste0(length(domain_only_ids), 
    ...
    if (length(subject_only_ids > 0)) {
        if (!bQuiet) {
            cli::cli_alert_info(paste0(length(subject_only_ids), 
    ...
    if (!is.null(vRemoval)) {
        n_na_zero <- dfOut %>% summarise(across(all_of(vRemoval), 
            ~sum(is.na(.x) | .x == 0))) %>% rowSums()
        if (n_na_zero > 0) {
            if (!bQuiet) {
                cli::cli_alert_info(paste0(n_na_zero, " row(s) in merged data have zero or NA values for columns: ", 
