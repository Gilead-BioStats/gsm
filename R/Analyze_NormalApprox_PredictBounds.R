function(
    dfTransformed, vThreshold = c(-3, -2, 2, 3), strType = "binary",
  ...
  vRange <- seq(min(dfTransformed$Denominator) - nStep, max(dfTransformed$Denominator) +
    nStep, by = nStep)
  if (strType == "binary") {
    dfBounds <- tidyr::expand_grid(
      Threshold = vThreshold,
      Denominator = vRange
    ) %>%
      filter(.data$Denominator >
        0) %>%
      mutate(
    ...
    dfBounds <- tidyr::expand_grid(
      ...
      filter(.data$Numerator >=
        0) %>%
      select(
        "Threshold", "Denominator", "LogDenominator",
        "Numerator", "Metric"
      )
  }
  return(dfBounds)
