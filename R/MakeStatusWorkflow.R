function (lResults, dfConfigWorkflow) {
    ...
    status_workflow <- dfConfigWorkflow %>% full_join(workflow_parse_status, 
        by = "workflowid") %>% mutate(status = ifelse(is.na(.data$status), 
        FALSE, .data$status)) %>% left_join(workflow_parse_warnings, 
