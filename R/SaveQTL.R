function (lSnapshot, strPath = "./inst/qtl_dummy_data/dummyqtldata.csv", 
    bQuiet = TRUE) {
    if (file.exists(strPath) & lSnapshot$bStatus) {
        ...
        utils::write.csv(qtl_all, paste0(tools::file_path_sans_ext(strPath), 
    ...
    if (!bQuiet) {
        cli::cli_alert_success(paste0("File: ", paste0(tools::file_path_sans_ext(strPath), 
            " ", Sys.Date(), ".csv"), " created."))
    }
}
