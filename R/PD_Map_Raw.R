...
#'
#' @param dfPD  PD dataset with columns USUBJID and rows for each Protocol Deviation 
...
PD_Map_Raw <- function( dfPD = NULL, dfTOS = NULL ){
    stopifnot(
        ...
        "USUBJID column not found in dfPD"="USUBJID" %in% names(dfPD),
        "SubjectID, SiteID and TimeOnStudy columns not found in dfTOS"=all(c("SubjectID","SiteID","Exposure") %in% names(dfTOS))
    )

    dfInput <-  dfTOS %>% 
        rowwise() %>%
        mutate(Count = sum(dfPD$USUBJID==.data$SubjectID)) %>% 
