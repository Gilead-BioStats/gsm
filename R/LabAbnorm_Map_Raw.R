...
#'
#' @examples
#'  dfInput <- LabAbnorm_Map_Raw(clindata::raw_ae, clindata::rawplus_rdsl)
#'
#' @import dplyr
#'
#' @export

LabAbnorm_Map_Raw <- function( dfLab, dfRDSL, strExposureCol="TimeOnTreatment"){
    stopifnot(
        "ae dataset not found"=is.data.frame(dfLab),
        "RDSL dataset is not found"=is.data.frame(dfRDSL),
        "SUBJID column not found in dfAE"="SUBJID" %in% names(dfLab),
        ...
        "NAs found in SUBJID column of dfAE" = all(!is.na(dfLab$SUBJID)),
