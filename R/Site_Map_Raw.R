function(
    dfs = list(dfSITE = clindata::ctms_site, dfSUBJ = clindata::rawplus_dm),
    lMapping = gsm::Read_Mapping(c("ctms", "rawplus")), dfConfig = gsm::config_param) {
  status_site <- dfs$dfSITE %>%
    mutate(
      siteid = as.character(.data[[lMapping$dfSITE[["strSiteCol"]]]]),
      invname = paste0(
    ...
    select(
      studyid = lMapping$dfSITE[["strStudyCol"]], "siteid",
  ...
  status_site <- status_site %>% select(all_of(gsm::rbm_data_spec %>%
    filter(
      .data$System == "Gismo", .data$Table == "status_site",
