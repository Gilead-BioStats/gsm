function (results = NULL, workflow = list(), threshold = NULL, 
    yaxis = "score", selectedGroupIDs = NULL, addSiteSelect = TRUE, 
    width = NULL, height = NULL, elementId = NULL, siteSelectLabelValue = NULL) {
    results <- results %>% dplyr::mutate(across(everything(), 
        as.character)) %>% dplyr::rename_with(tolower)
    ...
    x <- list(results = results, workflow = workflow, threshold = threshold, 
    ...
    htmlwidgets::createWidget(name = "Widget_BarChart", x, width = width, 
