function(dfs = list(
           dfSUBJ = clindata::rawplus_dm, dfQUERY = clindata::edc_queries,
           dfDATACHG = clindata::edc_data_points
         ), lMapping = gsm::Read_Mapping(c(
           "edc",
           "rawplus"
  ...
  checks <- CheckInputs(
    context = "QueryRate_Map_Raw", dfs = dfs,
  ...
  if (checks$status) {
    ...
    dfInput <- dfQUERY_mapped %>%
      ...
      ungroup() %>%
      mutate(Count = tidyr::replace_na(
        .data$Count,
        0
      ), DataPoint = tidyr::replace_na(
        .data$DataPoint,
        0
      )) %>%
      gsm::MergeSubjects(dfSUBJ_mapped,
        vFillZero = "Count",
        vRemoval = "DataPoint", bQuiet = bQuiet
      ) %>%
      mutate(Rate = .data$Count / .data$DataPoint) %>%
      select(
        any_of(c(names(dfSUBJ_mapped))), "Count",
        "DataPoint", "Rate"
      ) %>%
