function(df, mapping, spec, bQuiet = TRUE) {
  tests_if <- list(
    ...
    spec_is_list = list(status = NA, warning = NA), mapping_is_list = list(
      status = NA,
      warning = NA
    ), mappings_are_character = list(
      status = NA,
      warning = NA
    ), has_expected_columns = list(
      status = NA,
      warning = NA
    ), columns_have_na = list(
      status = NA,
      warning = NA
    ), columns_have_empty_values = list(
      status = NA,
      warning = NA
    ), cols_are_unique = list(
      status = NA,
      warning = NA
    )
  )
  ...
  if (tests_if$spec_is_list$status) {
    if (!all(spec$vRequired %in% names(mapping))) {
      ...
      tests_if$has_required_params$warning <- paste0(
        "\"mapping\" does not contain required parameters: ",
        missing_params
      )
    } else {
    ...
    if (!all(is.character(colNames))) {
      ...
      tests_if$mappings_are_character$warning <- paste0(
        warning,
    ...
    if (!all(colNames %in% names(df))) {
      ...
      warning <- paste0(
        "the following columns not found in df: ",
        warning_cols
      )
  ...
  if (tests_if$has_expected_columns$status) {
    ...
    if (!is.null(spec$vUniqueCols)) {
      ...
      if (any(dupes)) {
        ...
        warning <- paste0(
          "Unexpected duplicates found in column: ",
          names(dupes)
        )
