function (dfs = list(dfSUBJ = clindata::rawplus_dm, dfDATAENT = clindata::edc_data_pages), 
    ...
    checks <- CheckInputs(context = "DataEntry_Map_Raw", dfs = dfs, 
        bQuiet = bQuiet, mapping = lMapping)
    if (checks$status) {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn DataEntry_Map_Raw}")
        dfDATAENT_mapped <- dfs$dfDATAENT %>% select(SubjectID = lMapping[["dfDATAENT"]][["strIDCol"]], 
        ...
        dfInput <- dfDATAENT_mapped %>% mutate(Count = if_else(.data$DataEntryLag > 
            nMaxDataEntryLag, 1, 0), Total = 1) %>% group_by(.data$SubjectID) %>% 
