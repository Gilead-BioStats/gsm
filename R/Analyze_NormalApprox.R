function(dfTransformed, strType = "binary", bQuiet = TRUE) {
  stopifnot(
    ...
    `One or more of these columns not found: GroupID, Denominator, Numerator, Metric` = all(c(
      "GroupID",
      "Denominator", "Numerator", "Metric"
    ) %in% names(dfTransformed)),
    `NA value(s) found in GroupID` = all(!is.na(dfTransformed[["GroupID"]])),
    `strType is not 'binary' or 'rate'` = strType %in% c(
      "binary",
      "rate"
    )
  )
  if (strType == "binary") {
    dfScore <- dfTransformed %>% mutate(
  ...
  dfAnalyzed <- dfScore %>%
    select("GroupID", "Numerator",
      "Denominator", "Metric",
      OverallMetric = "vMu", Factor = "phi",
      Score = "z_i"
    ) %>%
    arrange(.data$Score)
  if (!bQuiet) {
