function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "QueryAge_Assess.yaml", 
    ...
    lChecks <- gsm::CheckInputs(context = "QueryAge_Assess", 
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn QueryAge_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn QueryAge_Assess}")
        if (!bQuiet) 
        ...
        dfConfig <- MakeDfConfig(strMethod = strMethod, strGroup = strGroup, 
            strAbbreviation = "OQRY", strMetric = "Outstanding Query Rate", 
            strNumerator = "Queries Open > 30 Days", strDenominator = "Total Queries", 
            vThreshold = vThreshold)
        if (strMethod != "Identity") {
            ...
            lCharts$scatterJS <- gsm::Widget_ScatterPlot(results = lData$dfSummary, 
                ...
                elementId = "queryAgeAssessScatter", siteSelectLabelValue = strGroup)
            if (!bQuiet) 
        ...
        lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "metric", elementId = "queryAgeAssessMetric", 
            siteSelectLabelValue = strGroup)
        lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "score", elementId = "queryAgeAssessScore", 
            siteSelectLabelValue = strGroup)
        if (!bQuiet) 
