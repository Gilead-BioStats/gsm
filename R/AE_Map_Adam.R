function (dfs = list(dfADSL = safetyData::adam_adsl, dfADAE = safetyData::adam_adae), 
    ...
    checks <- CheckInputs(context = "AE_Map_Adam", dfs = dfs, 
    ...
    if (checks$status) {
        ...
        dfInput <- dfs$dfADSL %>% mutate(SubjectID = .data[[lMapping$dfADSL$strIDCol]], 
            Exposure = as.numeric(.data[[lMapping$dfADSL$strEndCol]] - 
                .data[[lMapping$dfADSL$strStartCol]]) + 1) %>% 
            rowwise() %>% mutate(Count = sum(dfs$dfADAE[[lMapping$dfADAE$strIDCol]] == 
            .data$SubjectID), Rate = .data$Count/.data$Exposure) %>% 
            ungroup() %>% select("SubjectID", any_of(c(SiteID = lMapping[["dfADSL"]][["strSiteCol"]], 
