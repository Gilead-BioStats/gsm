function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "DataEntry_Assess.yaml", 
    ...
    lChecks <- gsm::CheckInputs(context = "DataEntry_Assess", 
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn DataEntry_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn DataEntry_Assess}")
        if (!bQuiet) 
        ...
        dfConfig <- MakeDfConfig(strMethod = strMethod, strGroup = strGroup, 
            strAbbreviation = "ODAT", strMetric = "Outstanding Data Entry Rate", 
            strNumerator = "Outstanding Data Pages", strDenominator = "Total Data Pages", 
            vThreshold = vThreshold)
        if (strMethod != "Identity") {
            ...
            lCharts$scatterJS <- gsm::Widget_ScatterPlot(results = lData$dfSummary, 
                ...
                elementId = "dataEntryAssessScatter", siteSelectLabelValue = strGroup)
            if (!bQuiet) 
        ...
        lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "metric", elementId = "dataEntryAssessMetric", 
            siteSelectLabelValue = strGroup)
        lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "score", elementId = "dataEntryAssessScore", 
            siteSelectLabelValue = strGroup)
        if (!bQuiet) 
