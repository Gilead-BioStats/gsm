function(
    dfs = list(
      dfSUBJ = clindata::rawplus_dm, dfSTUDCOMP = clindata::rawplus_studcomp,
  ...
  checks <- gsm::CheckInputs(context = paste0(
    "Disp_Map_Raw",
    "_", strContext
  ...
  if (checks$status) {
    ...
    dfInput <- gsm::MergeSubjects(
      ...
      dfSUBJ = dfSUBJ_mapped, bQuiet = bQuiet
    ) %>%
      mutate(Count = ifelse(is.na(.data$Count),
        0, .data$Count
      ), Total = 1) %>%
      select(
