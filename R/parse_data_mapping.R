function (content = NULL, file = NULL) {
    ...
    for (domain in domains) {
        ...
        mapping_tbl <- mapping %>% tibble::enframe(name = "col_key", 
            value = "col_value") %>% dplyr::mutate(col_value = as.character(.data$col_value)) %>% 
            tidyr::unnest(cols = "col_value") %>% dplyr::mutate(domain = domain) %>% 
