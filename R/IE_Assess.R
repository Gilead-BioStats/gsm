function (dfInput, nThreshold = 0.5, lMapping = yaml::read_yaml(system.file("mappings", 
    "IE_Assess.yaml", package = "gsm")), strGroup = "Site", nMinDenominator = NULL, 
    bQuiet = TRUE) {
    ...
    lChecks <- CheckInputs(context = "IE_Assess", dfs = list(dfInput = dfInput), 
        mapping = lMapping, bQuiet = bQuiet)
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn IE_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn IE_Assess}")
        if (!bQuiet) 
        ...
        lData$dfAnalyzed <- Analyze_Identity(lData$dfTransformed, 
            bQuiet = bQuiet)
        if (!bQuiet) 
        ...
        dfConfig <- MakeDfConfig(strMethod = "Identity", strGroup = strGroup, 
            strAbbreviation = "IE", strMetric = "Inclusion/Exclusion Issues", 
            strNumerator = "Inclusion/Exclusion Issues", strDenominator = "", 
            vThreshold = nThreshold)
        lCharts$barMetric <- Visualize_Score(dfSummary = lData$dfSummary, 
            strType = "metric")
        lCharts$barScore <- Visualize_Score(dfSummary = lData$dfSummary, 
            strType = "score", vThreshold = nThreshold)
        lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "metric", elementId = "ieAssessMetric", 
            siteSelectLabelValue = strGroup)
        lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "score", elementId = "ieAssessScore", 
            siteSelectLabelValue = strGroup)
        if (!bQuiet) 
