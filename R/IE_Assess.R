function(
    dfInput, nThreshold = 0.5, lMapping = yaml::read_yaml(system.file("mappings",
      "IE_Assess.yaml",
      package = "gsm"
  ...
  lChecks <- CheckInputs(
    context = "IE_Assess", dfs = list(dfInput = dfInput),
  ...
  if (!lChecks$status) {
    ...
    lData$dfFlagged <- gsm::Flag(lData$dfAnalyzed, vThreshold = c(
      NA,
      nThreshold
    ))
    if (!bQuiet) {
    ...
    dfConfig <- MakeDfConfig(
      strMethod = "Identity", strGroup = strGroup,
