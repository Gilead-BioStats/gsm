function(lAssessments, bViewReport = FALSE) {
  allChecks <- purrr::map(names(lAssessments), function(assessment) {
    workflow <- lAssessments[[assessment]][["steps"]] %>%
      purrr::imap_dfr(~ bind_cols(
        step = .x[["name"]], domain = .x[["inputs"]],
        temp_index = .y
      )) %>%
      arrange(
        .data$temp_index,
    ...
    allChecks <- purrr::map(mapTheseSteps, function(step) {
      domains <- sort(names(step[!names(step) %in% c(
        "mapping",
        "spec", "status"
      )]))
  ...
  found_data <- purrr::map(names(lAssessments), ~ lAssessments[[.x]][["lData"]]) %>%
    ...
    unique()
  allChecks <- allChecks %>%
    mutate(notes = ifelse(!.data$domain %in%
      found_data, paste0(
      "Data not found for ", .data$assessment,
      " assessment"
    ), NA_character_)) %>%
    select(
      "assessment",
  ...
  check_cols <- allChecks %>%
    ...
    names()
  allChecks <- allChecks %>% mutate(across(
    all_of(check_cols),
    ~ ifelse(!is.na(notes), NA_character_, .)
  ), notes = ifelse(is.na(.data$notes),
    apply(allChecks[6:length(allChecks)], 1, function(x) {
      paste(x[!is.na(x)],
        collapse = "<br>"
      )
    }), .data$notes
  ), check = case_when(.data$check ==
    TRUE ~ 1, .data$check == FALSE ~ 2, is.na(.data$check) ~
    3), notes = ifelse(.data$check == 3, "Check not run.",
    .data$notes
  ))
  dfSummary <- allChecks %>%
    mutate(check = purrr::map(
      .data$check,
      rank_chg
    )) %>%
    select(
      "assessment", "step", "check",
      "domain", "notes"
    )
  if (!bViewReport) {
