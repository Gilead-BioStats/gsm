function(dfAnalyzed, vThreshold = NULL) {
  ...
  if (all(!is.na(vThreshold))) {
    stopifnot(`vThreshold must contain cutoff for moderate/high risks in two directions (i.e., vThreshold = c(-3, -2, 2, 3))` = vThreshold[1] <
      vThreshold[2], `vThreshold must contain cutoff for moderate/high risks in two directions (i.e., vThreshold = c(-3, -2, 2, 3))` = vThreshold[2] <
      vThreshold[3], `vThreshold must contain cutoff for moderate/high risks in two directions (i.e., vThreshold = c(-3, -2, 2, 3))` = vThreshold[3] <
      vThreshold[4])
  }
