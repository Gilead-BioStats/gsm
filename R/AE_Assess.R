#' AE Assessment using Poisson Regression
#' 
...
AE_Assess <- function( dfInput, dThreshold=NULL, cLabel="", method="poisson", bDataList=FALSE){
    ...
    if(method == "poisson"){
        ...
        lAssess$dfFlagged <- gsm::Flag( lAssess$dfAnalyzed , strColumn = 'residual', vThreshold =c(-5,5))
    } else if(method=="wilcoxon"){
        lAssess$dfAnalyzed <- gsm::Analyze_Wilcoxon( lAssess$dfTransformed) 
        lAssess$dfFlagged <- gsm::Flag( lAssess$dfAnalyzed ,  strColumn = 'residual', vThreshold =0.0001)
    }
