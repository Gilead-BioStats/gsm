function (dfSUBJ, dfConfig, lMapping, strUnit, strBy) {
    stopifnot(`studyid not found in dfConfig` = "studyid" %in% 
    ...
    if (strUnit == "participant" & strBy == "study") {
        enrolled <- dm %>% group_by(.data[[lMapping$dfSUBJ$strStudyCol]]) %>% 
            summarize(n_enrolled = n()) %>% pull(.data$n_enrolled)
    }
    else if (strUnit == "site" & strBy == "study") {
        enrolled <- dm %>% summarize(n_enrolled_sites = n_distinct(.data[[lMapping$dfSUBJ$strSiteCol]])) %>% 
            pull(.data$n_enrolled_sites)
    }
    else if (strUnit == "participant" & strBy == "site") {
        enrolled <- dm %>% group_by(SiteID = .data[[lMapping$dfSUBJ$strSiteCol]]) %>% 
            summarize(enrolled_participants = n()) %>% ungroup()
    }
    return(enrolled)
}
