...
MakeBounds <- function(
  ...
  stop_if_any(
    conditions = exprs(!length(strMetrics), !length(dSnapshotDate), !length(strStudyID)),
    messages = c(
      "`strMetrics` must not be `NULL`.",
      "`dSnapshotDate` must not be `NULL`.",
      "`strStudyID` must not be `NULL`."
    )
  )
