...
RunWorkflow <- function(
  ...
  if ("spec" %in% names(lWorkflow)) {
    ...
    log4r::info(logger, glue::glue("Checking data against spec"))
    ...
    log4r::info(logger, glue::glue("No spec found in workflow. Proceeding without checking data."))
  }
  ...
  for (step in lWorkflow$steps) {
    ...
    log4r::info(logger, glue::glue("Workflow Step {stepCount} of {length(lWorkflow$steps)}: `{step$name}`"))
    result <- RunStep(
      lStep = step,
    ...
    if (step$output %in% names(lData)) {
      ...
      log4r::warn(logger, glue::glue("Overwriting existing data in `lData`."))
    }
    ...
    if (is.data.frame(result)) {
      ...
      log4r::info(logger, glue::glue("{paste(dim(result),collapse='x')} data.frame saved as `lData${step$output}`."))
    } else {
      cli::cli_h3("{typeof(result)} of length {length(result)} saved as `lData${step$output}`.")
      log4r::info(logger, glue::glue("{typeof(result)} of length {length(result)} saved as `lData${step$output}`."))
    }
  ...
  if (bReturnResult) {
    if (is.data.frame(lWorkflow$lResult)) {
      ...
      log4r::info(logger, glue::glue("Returning results from final step: {paste(dim(lWorkflow$lResult),collapse='x')} data.frame`."))
    } else {
      cli::cli_h2("Returning results from final step: {typeof(lWorkflow$lResult)} of length {length(lWorkflow$lResult)}`.")
      log4r::info(logger, glue::glue("Returning results from final step: {typeof(lWorkflow$lResult)} of length {length(lWorkflow$lResult)}`."))
    }
    cli::cli_h1("Completed `{uid}` Workflow")
    log4r::info(logger, glue::glue("Completed `{uid}` Workflow"))
    ...
    if (!bKeepInputData) {
      ...
      log4r::info(logger, glue::glue("Keeping only workflow outputs in $lData: {names(lWorkflow$lData)}"))
    } else {
      cli::cli_alert_info("Keeping workflow inputs and outputs in $lData: {names(lWorkflow$lData)}")
      log4r::info(logger, glue::glue("Keeping workflow inputs and outputs in $lData: {names(lWorkflow$lData)}"))
    }
    cli::cli_h2("Returning full workflow object.")
    log4r::info(logger, glue::glue("Returning full workflow object."))
