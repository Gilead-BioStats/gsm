function(version = NULL) {
  ...
  meta_update <- c(
    "meta_param.csv", "meta_workflow.csv", "config_param.csv",
    "config_workflow.csv"
  )
  lMeta_update <- purrr::map(meta_update, ~ read.csv(here::here(
    "data-raw",
    .x
  ))) %>%
    purrr::map(~ .x %>% mutate(gsm_version = version)) %>%
    purrr::set_names(meta_update)
  purrr::iwalk(lMeta_update, ~ utils::write.csv(.x, file = paste0(here::here(
    "data-raw",
    .y
  )), row.names = FALSE))
