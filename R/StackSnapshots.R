function (cPath, lSnapshot = NULL, vFolderNames = NULL) {
    ...
    if (length(snapshots) > 0) {
        ...
        gsm_tables <- c("meta_param", "meta_workflow", "results_analysis", 
            "results_summary", "status_param", "status_site", 
            "status_study", "status_workflow")
        longitudinal_data <- gsm_tables %>% map(function(gsm_table) {
            gsm_data <- snapshots %>% purrr::map_dfr(function(snapshot) {
                file_path <- glue::glue("{snapshot}/{gsm_table}.csv")
                if (file.exists(file_path)) {
                  data <- file_path %>% read.csv(colClasses = "character") %>% 
                    mutate(gsm_analysis_date = as.Date(.data$gsm_analysis_date, 
                      "%Y-%m-%d"), snapshot_date = as.Date(.data$gsm_analysis_date, 
                      "%Y-%m-%d"))
                  return(data)
                }
                else {
        ...
        if (!is.null(lSnapshot)) {
            common_tables <- intersect(names(longitudinal_data), 
            ...
            for (common_table in common_tables) {
                common_columns <- intersect(names(longitudinal_data[[common_table]]), 
                ...
                for (common_column in common_columns) {
                  ...
                  if (expected_class != detected_class) {
                    longitudinal_data[[common_table]][[common_column]] <- match.fun(paste0("as.", 
                      expected_class))(longitudinal_data[[common_table]][[common_column]])
                  }
                }
        ...
        longitudinal_data$parameters <- longitudinal_data$meta_param %>% 
            left_join(longitudinal_data$status_param, by = join_by("workflowid", 
        ...
        if (any(c("gsm_version.x", "gsm_version.y") %in% names(longitudinal_data$parameters))) {
            ...
            versions_y <- unique(longitudinal_data$parameters$gsm_version.y) %>% 
                stats::na.omit()
            latest_version <- max(base::as.package_version(c(versions_x, 
                versions_y)))
            other_versions <- sort(unique(c(versions_x, versions_y)[!c(versions_x, 
                versions_y) %in% as.character(latest_version)]))
