function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "DataChg_Assess.yaml", 
    ...
    lChecks <- gsm::CheckInputs(context = "DataChg_Assess", dfs = list(dfInput = dfInput), 
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn DataChg_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn DataChg_Assess}")
        if (!bQuiet) 
        ...
        dfConfig <- MakeDfConfig(strMethod = strMethod, strGroup = strGroup, 
            strAbbreviation = "CDAT", strMetric = "Data Change Rate", 
            strNumerator = "Data Points with 1+ Change", strDenominator = "Total Data Points", 
            vThreshold = vThreshold)
        if (strMethod != "Identity") {
            ...
            lCharts$scatterJS <- gsm::Widget_ScatterPlot(results = lData$dfSummary, 
                ...
                elementId = "dataChgAssessScatter", siteSelectLabelValue = strGroup)
            if (!bQuiet) 
        ...
        lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfFlagged, 
            workflow = dfConfig, yaxis = "metric", elementId = "dataChgAssessMetric", 
            siteSelectLabelValue = strGroup)
        lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfFlagged, 
            workflow = dfConfig, yaxis = "score", elementId = "dataChgAssessScore", 
            siteSelectLabelValue = strGroup)
        if (!bQuiet) 
