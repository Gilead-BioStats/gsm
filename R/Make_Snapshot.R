function(
  ...
  lResults <- gsm::Study_Assess(
    lData = lData, lMapping = lMapping,
    lAssessments = lAssessments, bQuiet = bQuiet
  ...
  if (length(grep("qtl", names(lResults))) > 0) {
    ...
    results_analysis <- data.frame(
      studyid = NA, workflowid = NA,
      param = NA, value = NA, gsm_analysis_date = NA
    )
  }
  status_study <- Study_Map_Raw(dfs = list(
    dfSTUDY = lMeta$meta_study,
    dfSUBJ = lData$dfSUBJ
  ), lMapping = lMapping, dfConfig = lMeta$config_param)
  status_site <- Site_Map_Raw(dfs = list(
  ...
  snapshot <- list(
    lSnapshotDate = gsm_analysis_date, lSnapshot = lSnapshot,
    lStudyAssessResults = lResults, lInputs = list(
      lMeta = lMeta,
