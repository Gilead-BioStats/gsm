function(
    domain, mapping = NULL, mapping_path = "./inst/mappings/",
    spec = NULL, spec_path = "./inst/specs/", out_path = "./man/md/",
    header = "# Data specification") {
  if (is.null(mapping)) {
    mapping <- gsm::parse_data_mapping(file = paste0(
      mapping_path,
      domain, ".yaml"
    ))
    if (is.null(mapping)) {
  ...
  if (is.null(spec)) {
    spec <- gsm::parse_data_spec(file = paste0(
      spec_path,
      domain, ".yaml"
    ))
    if (is.null(spec)) {
  ...
  table <- mapping %>%
    dplyr::right_join(spec, c(
      "domain",
      "col_key"
    )) %>%
    mutate(col_value = ifelse(is.na(.data$col_value) &
  ...
  col_name_dict <- c(
    domain = "Domain", col_key = "Column Key",
