function (dfInput, vThreshold = NULL, strMethod = "NormalApprox", 
    lMapping = yaml::read_yaml(system.file("mappings", "QueryRate_Assess.yaml", 
    ...
    lChecks <- gsm::CheckInputs(context = "QueryRate_Assess", 
        dfs = list(dfInput = dfInput), mapping = lMapping, bQuiet = bQuiet)
    if (is.null(vThreshold)) {
        vThreshold <- switch(strMethod, NormalApprox = c(-3, 
            -2, 2, 3), Poisson = c(-7, -5, 5, 7), Identity = c(6e-05, 
            0.01))
    }
    ...
    if (!lChecks$status) {
        if (!bQuiet) 
            cli::cli_alert_warning("{.fn QueryRate_Assess} did not run because of failed check.")
        return(list(lData = NULL, lCharts = NULL, lChecks = lChecks))
    }
    else {
        if (!bQuiet) 
            cli::cli_h2("Initializing {.fn QueryRate_Assess}")
        if (!bQuiet) 
        ...
        lData$dfTransformed <- gsm::Transform_Rate(dfInput = dfInput, 
            ...
            strDenominatorCol = "DataPoint", bQuiet = bQuiet)
        if (!bQuiet) 
        ...
        if (strMethod == "NormalApprox") {
            ...
            lData$dfAnalyzed <- gsm::Analyze_Poisson(dfTransformed = lData$dfTransformed, 
                bQuiet = bQuiet)
            lData$dfBounds <- gsm::Analyze_Poisson_PredictBounds(dfTransformed = lData$dfTransformed, 
        ...
        dfConfig <- MakeDfConfig(strMethod = strMethod, strGroup = strGroup, 
            strAbbreviation = "QRY", strMetric = "Query Rate", 
            strNumerator = "Total Queries", strDenominator = "Total Data Points", 
            vThreshold = vThreshold)
        if (strMethod != "Identity") {
            ...
            lCharts$scatterJS <- gsm::Widget_ScatterPlot(results = lData$dfSummary, 
                ...
                elementId = "queryRateAssessScatter", siteSelectLabelValue = strGroup)
            if (!bQuiet) 
        ...
        lCharts$barMetricJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "metric", elementId = "queryRateAssessMetric", 
            siteSelectLabelValue = strGroup)
        lCharts$barScoreJS <- gsm::Widget_BarChart(results = lData$dfSummary, 
            workflow = dfConfig, yaxis = "score", elementId = "queryRateAssessScore", 
            siteSelectLabelValue = strGroup)
        if (!bQuiet) 
