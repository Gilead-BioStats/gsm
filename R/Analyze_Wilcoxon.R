...
Analyze_Wilcoxon <- function(dfTransformed , strOutcome = "Rate") {
    ...
    wilcoxon_model <- function(site){
        form <- stats::as.formula(paste0(strOutcome," ~ as.character(SiteID) =='", site,"'"))
        stats::wilcox.test(form, exact = FALSE, conf.int = TRUE, data=dfTransformed)
    }

    dfAnalyzed <- dfTransformed %>%
        dplyr::mutate(model = purrr::map(.data$SiteID, wilcoxon_model)) %>%
        dplyr::mutate(summary = purrr::map(.data$model, broom::glance)) %>%
        tidyr::unnest(summary) %>%
        dplyr::mutate(Estimate = .data$estimate *-1) %>%
        dplyr::rename(PValue = .data[['p.value']]) %>%
        dplyr::arrange(.data$PValue) %>%
        dplyr::select(.data$SiteID, .data$N, .data$TotalCount, .data$TotalExposure, .data$Rate, .data$Estimate, .data$PValue)
