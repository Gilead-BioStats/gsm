...
for (i in unique(params$dfResults$MetricID)) {
  ...
  if (grepl("0010", i)) {
    ...
    avg_dataent_lag <- params$dfGroups$Value[params$dfGroups$Param == 'AvgDataEntryLag'] %>% as.numeric() %>% round(3)
    avg_dataent_lag_over10 <- params$dfGroups$Value[params$dfGroups$Param == 'AvgDataEntryLag_over10'] %>% as.numeric() %>% round(3)

    glue("
    <div style = 'margin-top: 1em;'>As of {setup$SnapshotDate}, {setup$StudyID} has an average Data Entry Lag of {avg_dataent_lag} days, and those that were flagged as lagging at least 10 days have an average lag time of {avg_dataent_lag_over10} days.</div>") %>% cat()
  }
