...
setup <- Report_Setup(dfStudy =  params$dfStudy, dfMetrics = params$dfMetrics)
...
htmltools::div(
  ...
  Widget_SiteOverview(
    dfSummary=dfSummary %>% rename_with(tolower) %>% mutate(workflowid=metricid), 
    dfSite=dfSite
  )
)
```
...
for (i in seq_along(lCharts)) {
  ...
  if(!is.null(dfMetrics)){
    MetricName <- dfMetrics$metric[dfMetrics$workflowid == MetricID]
  } else { 
    MetricName <- MetricID
  }
  ...
  cat(Report_MetricTable(dfSummary = dfMetricSummary, dfSite= dfSite))
