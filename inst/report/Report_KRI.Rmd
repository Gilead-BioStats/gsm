...
# Remove sites whose metrics have no flags 
dfResults_atleastoneflag <- params$dfResults %>% 
  group_by(GroupID, MetricID) %>% 
  mutate(flagsum = sum(Flag)) %>% # na.rm = TRUE depending on meaning
  ungroup() %>% 
...
FlagOverViz <- Widget_FlagOverTime(
  dfResults = dfResults_atleastoneflag,
  dfMetrics = params$dfMetrics,
  strGroupLevel = setup$GroupLevel,
  strFootnote = "*Only sites with at least one flag are shown."
)

overview_viz <- list(groupOverviewJS = GroupOverViz, flagOverTimeJS = FlagOverViz)
