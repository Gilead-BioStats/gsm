...
if(length(unique((params$dfResults)$SnapshotDate)) > 1){
  # Only include risk signals flagged at the most recent snapshot.
  dfResultsFlagged <- FilterByFlags(
    params$dfResults,
  ...
  FlagOverViz <- Widget_FlagOverTime(
    ...
    dfMetrics = params$dfMetrics,
    strGroupLevel = setup$GroupLevel,
    strFootnote = "*Includes risk signals flagged at the most recent snapshot only."
  )
