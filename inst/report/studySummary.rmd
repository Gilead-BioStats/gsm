...
library(tidyverse)
library(kableExtra)
allFindings <- params$assessments %>% map(~.x$dfSummary) %>% bind_rows()
gsm::Overview_Table(allFindings)%>%
    kbl(escape=FALSE) %>% 
    kable_styling()
```
...
for (assessment in params$assessments){
    ...
    cat("<h2 style='border-top:2px solid black; border-bottom:2px solid black; text-align:center; padding:3px;'> ", assessment$Label, " Details</h2>")
