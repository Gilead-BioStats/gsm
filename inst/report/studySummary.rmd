...
details <- assessment$dfFlagged %>%
  filter(Flag !=0) %>% 
  {if("TotalCount" %in% names(.)) arrange(., match(TotalCount, flagged$Score)) else .} %>% 
  {if("PValue" %in% names(.)) arrange(., match(PValue, flagged$Score)) else .} %>% 
  {if("Residuals" %in% names(.)) arrange(., match(Residuals, flagged$Score)) else .} 
