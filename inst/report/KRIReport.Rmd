...
if (exists('status_study', where = params) && !is.null(params$status_study)) {
  ...
  study_status_table <- params$status_study %>% 
    t() %>% 
  as.data.frame() %>% 
  rownames_to_column() %>% 
  setNames(c("Parameter", "Value")) %>% 
  mutate(
    Value = prettyNum(Value, drop0trailing = TRUE)
  ) %>% 
  left_join(
    paramDescription,
...
gt::gt(study_status_table, id = "study_table") %>% 
  gt::tab_options(
    table.font.size = 14,
...
#  add analysis date if dfStudy metadata is provided
gsm_analysis_date <- study_status_table %>% 
  filter(Parameter == "gsm_analysis_date") %>% 
  pull("Value")

 glue::glue(
'
---
date: "Snapshot Date: {gsm_analysis_date}"
---
')
