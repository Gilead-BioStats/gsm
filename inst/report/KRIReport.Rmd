...
summary_table <- map(assessment, function(kri) {
  ...
  if(nrow(dfFlagged) > 0 & !all(dfFlagged$Flag == 0)) {
...
## StudyID
study_id <- purrr::map_chr(study, ~.x$lData$dfInput$StudyID %>% unique()) %>% unique()
```
...
Overview for Study: **`r purrr::map_chr(study, ~.x$lData$dfInput$StudyID %>% unique()) %>% unique()`**
