...
This vignette walks users through the mechanics of the `Assess()` functions within the `{gsm}` package, which leverages Key Risk Indicators (KRIs) and thresholds to conduct study-level and site-level Risk Based Monitoring for clinical trials. The `Assess()` functions provide data frames, visualization, and metadata to be used in reporting and error checking. The image below illustrates the supporting functions that are intrinsic to the workflow of the `Assess()` functions.
...
-   `Transform_Rate()`: Calculates cumulative rate of the Event of Interest per site. In our example, the default metric for AEs is the number of AEs reported over the course of treatment per site.
...
-   `GroupID` = Group Identifier (i.e., Site ID)
-   `Numerator` = Cumulative Count of Incidence of the Event of Interest (in this example, number of AEs reported)
-   `Denominator` = Cumulative Count of the Total Time on Treatment (in days)
-   `Metric` = Rate of Event of Interest (in this example, number of AEs reported over the course of treatment in days)
...
The data frame `dfAnalyzed` is derived from `dfTransformed` using one of the following functions built from a specific statistical model (depending on the assessment being run):

-   `Analyze_Fisher()`: Uses Fisher's Exact Test to determine if there are nonrandom associations between a site and a given KRI
-   `Analyze_Identity()`: Used in the data pipeline between `Transform()` and `Flag()` functions to rename KRI and Score columns
-   `Analyze_Poisson()`: Uses a Poisson model to describe the distribution of events in the overall site population, i.e., determine how many times an event is likely to occur at a site over a specified period of time
...
-   `GroupID` = Group Identifier (i.e., Site ID)
-   `Numerator` = Cumulative Count of Incidence of the Event of Interest (in this example, number of AEs reported); Carried from `dfTransformed`
-   `Denominator` = Cumulative Count of the Total Time on Treatment (in days); Carried from `dfTransformed`
-   `Metric` = Rate of Event of Interest (in this example, number of AEs reported over the course of treatment in days); Carried from `dfTransformed`
...
-   `Flag_Fisher()`: Default flagging function when `Analyze_Fisher()` function is used for an assessment
-   `Flag_Poisson()`: Default flagging function when `Analyze_Poisson()` function is used for an assessment
-   `Flag()`: Default flagging function when `Analyze_Identity()` function is used for an assessment

The resulting `dfFlagged` data frame will contain site-level analysis results data with flagging incorporated based on a pre-specified statistical threshold to highlight possible outliers. The default threshold for the AE Poisson model is (-7, -5, 5, 7), so using our example AE data, `dfFlagged` contains the following columns:

-   `GroupID` = Group Identifier (i.e., Site ID)
-   `Numerator` = Cumulative Count of Incidence of the Event of Interest (in this example, number of AEs reported); Carried from `dfAnalyzed`
-   `Denominator` = Cumulative Count of the Total Time on Treatment (in days); Carried from `dfAnalyzed`
-   `Metric` = Rate of Event of Interest (in this example, number of AEs reported over the course of treatment in days); Carried from `dfAnalyzed`
...
-   `Flag` = Flag Indicating Possible Statistical Outliers
...
The data frame `dfSummary` is derived from `dfFlagged` using the `Summarize()` function. The resulting `dfSummary` data frame will contain the most relevant columns from `dfFlagged` with data sorted in a meaningful way to provide a concise overview of the assessment. Flagged sites will appear earlier than unflagged sites, with the more "extreme" outliers displayed first. These columns include:
...
-   `Visualize_Scatter`: Creates scatterplot of Total Exposure (days, log scale) vs Total Incidence of Event of Interest (linear scale); Each data point represents one site; Outliers are plotted in red and the associated site label is present; Only created when statistical method is **not** defined as `identity`
...
For an AE assessment, a scatterplot is created using `dfTransformed` to define bounds and `dfFlagged` to plot the data. Using our example AE data, we see the following scatterplot:
