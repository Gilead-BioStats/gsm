...
-   `CustomGroupID` = Custom Group Identifier
...
-   `OverallMetric` = Group Total Rate of Event(s) of Interest
-   `Factor` = 
...
-   `GroupID` = Group Identifier (default is Site ID).
-   `Numerator` = Number of Event(s) of Interest (in this example, number of AEs reported over the course of treatment in days).
-   `Denominator` = Represents the group total for the risk that is being analyzed (in this example, total number of days of exposure at site).
-   `Metric` = Rate of Event(s) of Interest (in this example, number of AEs reported over the course of treatment in days).
-   `Score` = Calculated Residual (per site).
-   `Flag` = Flag Indicating Possible Statistical Outliers; Valid values for this variable include -2, -1, 0, 1, and 2, which determine the "extremeness" of the outlier. -2 and 2 represent more extreme outliers, -1 and 1 represent less extreme outliers, and 0 represents a non-outlier.
...
-   `Analyze_NormalApprox()`: Uses Funnel Plot Analysis with Normal Approximation for Binary and Rate outcomes
...
-   `Analyze_QTL()`: Uses exact binomial or Poisson test for Binary and Rate outcomes
...
There are two types of standard visualizations that are created by every `Assess()` function: `ggplot` visualizations and `htmlwidget`/`JavaScript` interactive visualizations.

#### Interactive Visualizations

Interactive visualizations prefixed with `Widget_` are created using a custom JavaScript charting library, [`{rbm-viz}`](https://github.com/Gilead-BioStats/rbm-viz). `Widget_ScatterPlot()` and `Widget_BarChart()` contain a drop-down menu to highlight a specific group within the assessment, e.g., a specific Site within an Adverse Event Assessment. All interactive visualizations also include mouseover tooltips, which display data for the chart, scatter plot point, or cell that is hovered over.

-   `Widget_ScatterPlot()`: Creates a group-level scatter plot based on the output of a KRI analysis.
-   `Widget_BarChart()`: Creates a group-level bar chart based on the output of a KRI analysis.
-   `Visualize_Workflow`: Provides a flowchart with a high-level overview of the data pipeline for a given assessment

#### Static Visualizations 

Static visualizations are created using `{ggplot2}`
...
All Assessments use the **Normal Approximation** by default. Other options include:
