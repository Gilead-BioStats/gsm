...
<!-- Next, let's run the same assessment above, but this time using non-standard data. For non-standard data, the user must perform data transformation/manipulation so that is can correctly pass into a mapping function. -->
...
<!-- ## Configure raw AE data for correct input of dfAE into mapping function -->
...
<!-- ## Configure raw subject-level data for correct input of dfSUBJ into mapping function -->
...
For this example, we will only keep subjects who belong to Site #27:
...
custom_mapping$dfSUBJ$strSiteVal <- "27"
