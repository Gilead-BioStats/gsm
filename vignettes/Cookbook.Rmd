...
For more information on of the {gsm} package see the [package homepage](https://silver-potato-cfe8c2fb.pages.github.io/). The [Data Pipeline Vignette](https://github.com/Gilead-BioStats/gsm/wiki/Data-Pipeline-Vignette) provides additional technical details including data specifications, and other technical details.
...
_*`lData` expects a named list. To see a list of default data frame names, run `names(clindata::mapping_rawplus)`_
...
Let's take a closer look at functionality that is built into `Study_Assess()`. As a practical example, a user may want to keep subjects from an assessment based on a some criteria (e.g., a specific site, region, etc.). 
...
custom_mapping <- clindata::mapping_rawplus
...
Study_Report(strAssessments = multiple_assessments, lMeta = list(label = "My Study"))
```
...
```
Here's an example of a report that shows no errors: 

![image](https://user-images.githubusercontent.com/40671730/169323890-b084efdb-2eea-43e9-b86b-4cf5770689bd.png)
...
As mentioned in the [Data Pipeline Vignette](https://github.com/Gilead-BioStats/gsm/wiki/Data-Pipeline-Vignette#metadata-technical-specifications), `Study_Assess()` triggers a workflow that uses pre-defined YAML specs that organizes all of the required metadata for a given assessment or set of assessments. 
