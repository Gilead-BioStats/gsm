...
###  `Reporting_Results`
  ...
  - Workflow used to create table: `3_reporting/Results.yaml`
  - Usage: Summarize KRI at the group level for reporting.
  - Structure:
...
### `Reporting_Bounds`
  ...
  - Inputs: `lAnalysis`, `strStudyID`, `dSnapshotDate`
  - Workflow used to create table: `3_reporting/Bounds.yaml`
...
###  `Reporting_Groups`
  ...
  - Workflow used to create table: `3_reporting/Groups.yaml`
...
###  `Reporting_Metrics`
  - Function used to create table: `gsm.mapping::MakeMetric()`
  - Inputs: `lWorkflows` - metadata for the corresponding kri(s) made with `MakeWorkflowList()`
  - Workflow used to create table: `3_reporting/Metrics.yaml`
...
The `lWorkflow` object is a named list of metadata and steps defining how each assessment should be run. By default, `MakeWorkflowList()` imports YAML specifications from `workflow/2_metrics`. Each item in `lWorkflow` expects the following parameters in the `steps` section:
...
For example, the default workflow for the AE assessment (`system.file("workflow/2_metrics/kri0001.yaml", package = "gsm")`) is shown below:

```{yaml, code = readLines(system.file("workflow/2_metrics/kri0001.yaml", package = "gsm")), eval = FALSE}
```
