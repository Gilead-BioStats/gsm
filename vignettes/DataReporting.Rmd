...
An more detailed explanation of the `Param`s for each group level can be found in the [Data Pipeline Vignette](DataPipeline.html) 
...
The reporting workflow requires that all metrics are stacked into a single data frame, `dfResults`. This stacked data frame is created by feeding the `lAnalysis` list from the analytics workflow into `BindResults()` along with the snapshot date and the study id.
...
lCharts$kri0001$scatterJS
```


### Step 3 - Generate Report

All of the components are created to generate the HTML report for the study we are working on. In order to generate this report and save it locally, simply feed `lCharts`, `dfResults`, `dfGroups`, `dfMetrics` and (optionally) a path to which the report will be saved (`strOutpath`) into `Report_KRI()` and the HTML output will be knit from the `Report_KRI.Rmd` template. 
...
Below, you will see a screenshot from the beginning of the report. All charts for all metrics that were included throughout the analytics and reporting workflows will be included in this report.
...
- `Widget_BarChart`:  Creates an interactive barchart visualization for Score or KRI. Charts are called `barScoreJS` or `barMetricJS` in the `lCharts` object.
