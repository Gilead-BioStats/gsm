...
```{r include = TRUE}
dfMetrics <- gsm.mapping::MakeMetric(lWorkflows = metrics_wf)
```
...
### Recap - Reporting Workflow 
  ...
  -   `dfGroups` created from CTMS data using `RunQuery()`, `gsm.mapping::MakeLongMeta()` and `bind_rows()`
  -   `dfMetrics` created from `lWorkflow` using `gsm.mapping::MakeMetric()`
  -   `dfResults` created from `lAnalysis$dfSummary` using `gsm.mapping::BindResults()`
  -   `dfBounds` created from `dfResults` using `gsm.mapping::MakeBounds()`
