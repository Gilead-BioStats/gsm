...
The `{gsm}` package provides a standardized workflow that leverages Key Risk Indicators (KRIs) and thresholds to conduct study-level Risk Based Monitoring (RBM) for clinical trials. This vignette provides an overview of the `{gsm}` data model. First, we provide an summary of the standardized data pipeline used to calculate KRIs and evaluate thresholds to set site-level flags. We also discuss workflows and reports that allow users to run multiple KRIs concurrently on a study.

As an overview, the data pipeline can be described as follows: clinical and operational data are analyzed using the `{gsm}` R package, which generates outputs that are passed to the Gismo web application. The Appendices of this document provide detailed data specifications for the required inputs in `{gsm}` and Gismo.

# KRI Data Pipeline

<img src="https://user-images.githubusercontent.com/3680095/169086356-e31ab9a5-c709-46cf-9bd9-68b7bdbba72b.png" alt="image" width="862"/>
...
Each assessment has an `Assess()` function that sequentially executes all 4 of the supporting functions and returns a list containing all 5 datasets listed above. Details on this can be found in the [Data Analysis Vignette](https://github.com/Gilead-BioStats/gsm/wiki/Data-Analysis-Vignette).

## Generating Input Data

Generating input data is outside the scope of the assessment pipeline. The specifications for input data (i.e., `dfInput`) are designed to be flexible enough so that this data frame can be generated from multiple clinical data standards (e.g., Raw, Raw+, or CDISC-compliant ADaM). Most assessments have `Map()` functions that can be used to generate input data. For example, the Adverse Event assessment has 2 `Map()` functions: - `AE_Map_Raw()`: creates input data from Raw+ data - `AE_Map_Adam()`: creates input data from CDISC-compliant ADaM data
...
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
| **Table\  | **Table Name**  | **Structure** | **Description**                          | Sample Data\              |
| Source**  |                 |               |                                          | Equivalent                |
+:==========+:================+:==============+:=========================================+===========================+
...
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
| RACT      | config_workflow | Study/\       | Study-specific workflow configuration    | clindata::config_workflow |
|           |                 | Workflow      |                                          |                           |
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
| RACT      | config_param    | Study/\       | Study-specific workflow parameterization | clindata::config_param    |
|           |                 | Workflow/\    |                                          |                           |
|           |                 | Parameter     |                                          |                           |
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
...
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
| GSM       | meta_params     | Workflow/\    | Parameter metadata for each workflow     | gsm::meta_param           |
|           |                 | Parameter     |                                          |                           |
+-----------+-----------------+---------------+------------------------------------------+---------------------------+
...
+============+=================+===================+===========================================+
| Status     | status_study    | Study             | Study metadata and status info            |
+------------+-----------------+-------------------+-------------------------------------------+
| Status     | status_site     | Study/Site        | Site metadata and status info             |
+------------+-----------------+-------------------+-------------------------------------------+
| Status     | status_workflow | Study/Workflow    | Workflow status info                      |
+------------+-----------------+-------------------+-------------------------------------------+
| Status     | status_param    | Study/Workflow/\  | Parameter status info                     |
|            |                 | Parameter         |                                           |
+------------+-----------------+-------------------+-------------------------------------------+
| Status     | status_schedule | Study/Date        | Dates for QRM Snapshots                   |
+------------+-----------------+-------------------+-------------------------------------------+
| Result     | results_summary | Study/Workflow/\  | Full analytics results for all workflows  |
|            |                 | Group             |                                           |
+------------+-----------------+-------------------+-------------------------------------------+
| Result     | results_bounds  | Study/Workflow/\  | x/y values for boundaries on scatter plot |
|            |                 | Threshold/x-value |                                           |
+------------+-----------------+-------------------+-------------------------------------------+
| Metadata   | meta_workflow   | Workflow          | Metadata for each workflow                |
+------------+-----------------+-------------------+-------------------------------------------+
| Metadata   | meta_param      | Workflow/\        | Parameter metadata for each workflow      |
|            |                 | Parameter         |                                           |
+------------+-----------------+-------------------+-------------------------------------------+
