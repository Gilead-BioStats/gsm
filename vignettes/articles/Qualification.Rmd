...
spec_list <- spec_list[1:5]
spec_list <- paste0("~/gsm/tests/testqualification/qualification/", spec_list)

scrape <- map_df(spec_list, function(x) {
    testResultsRaw <- invisible(testthat::test_file(x, reporter = testthat::SilentReporter)[[1]])
...
testResultsIndividual <- scrape %>%
  as_tibble() %>%
  rename(`Test Name` = test) %>%
  # group_by(file, context, `Test Name`) %>%
  summarise(`Number of Tests` = length(unique(name)),
