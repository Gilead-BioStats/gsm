...
test_that("Widget_TimeSeries() outputs data properly", {
  ...
  expect_equal(
    names(plot), c("x", "width", "height", "sizingPolicy", "dependencies",
  ...
  # dfSummary ---------------------------------------------------------------------------
  dfSummary_names <- c("studyid", "groupid", "numerator", "denominator", "metric",
  ...
  # lLabels -----------------------------------------------------------------------------
  lLabel_names <- c("workflowid", "group", "abbreviation", "metric", "numerator","denominator",
                    "outcome", "model", "score", "data_inputs", "data_filters", "gsm_analysis_date")

  expect_equal(lLabel_names, names(plot$x$lLabels))

  # dfParams ----------------------------------------------------------------------------
  dfParams_names <- c("workflowid", "param", "index", "gsm_analysis_date",
