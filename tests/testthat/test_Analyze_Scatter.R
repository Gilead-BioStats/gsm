...
test_that("output created as expected and has correct structure",{
    ...
    ae_anly <- Analyze_Scatter(ae_prep)
...
test_that("incorrect inputs throw errors",{
    expect_error(Analyze_Scatter(list()))
    expect_error(Analyze_Scatter("Hi"))
})


test_that("error given if required column not found",{
    ae_prep <- Transform_EventCount( ae_input, strCountCol = 'Count', strExposureCol = "Exposure" )
    expect_error(Analyze_Scatter(ae_prep %>% select(-SiteID)))
    expect_error(Analyze_Scatter(ae_prep %>% select(-N)))
    expect_error(Analyze_Scatter(ae_prep %>% select(-TotalCount)))
    expect_error(Analyze_Scatter(ae_prep %>% select(-TotalExposure)))
    expect_error(Analyze_Scatter(ae_prep %>% select(-Rate)))
})
