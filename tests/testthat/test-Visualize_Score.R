test_that("Visualize_Score stops on incorrect input types", {
  expect_error(
    Visualize_Score(reportingResults, strTitle = 123),
    "strTitle must be character"
  )
  expect_error(
    Visualize_Score(reportingResults, bFlagFilter = "TRUE"),
  ...
  expect_error(
    Visualize_Score(reportingResults, strType = "invalid"),
    "strType must be 'Metric' or 'Score'"
  )
  expect_error(
    Visualize_Score(reportingResults, strType = c("Metric", "Score")),
