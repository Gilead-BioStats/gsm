...
test_that("output is created as expected", {
  df <- is_mapping_valid(df = dfSUBJ, mapping = mapping_rdsl, vRequiredParams = c("strIDCol", "strSiteCol", "strExposureCol"))

  expect_type(df, "list")
  expect_equal(names(df), c("status", "tests_if"))

  expect_equal(names(df$tests_if), c("is_data_frame", "has_required_params", "mapping_is_list",
...
test_that("NA values are caught", {
  ...
  df <- is_mapping_valid(dfSUBJ, mapping = mapping_rdsl, vRequiredParams = c("strIDCol", "strSiteCol", "strExposureCol"))
...
test_that("NA values are ignored when specified in vNACols", {
  df <- is_mapping_valid(df = dfSUBJ, mapping = mapping_rdsl, vNACols = "strExposureCol", vRequiredParams = c("strIDCol", "strSiteCol", "strExposureCol"))
