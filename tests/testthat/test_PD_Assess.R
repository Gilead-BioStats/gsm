...
test_that("incorrect inputs throw errors",{
  ...
  expect_error(PD_Assess(pd_input, cMethod=c("wilcoxon", "poisson")))
  expect_error(PD_Assess(pd_input, vThreshold = "A"))
  expect_error(PD_Assess(pd_input, vThreshold = 1))
  expect_error(PD_Assess(pd_input, cLabel = iris))
})

test_that("Confirm expected functionality for PD_Assess input clabel ",{
  rdsl<-clindata::rawplus_rdsl %>% filter(RandFlag=="Y")
  pd_input <- PD_Map_Raw(dfPD = clindata::raw_protdev,dfRDSL = rdsl)
  pd_summary_df <- PD_Assess(pd_input, cLabel = "test_label")
  expect_equal(pd_summary_df[1,"Label", drop = TRUE], 'test_label')
})

test_that("Confirm expected functionality for PD_Assess input cMethod ",{
  rdsl<-clindata::rawplus_rdsl %>% filter(RandFlag=="Y")
  pd_input <- PD_Map_Raw(dfPD = clindata::raw_protdev,dfRDSL = rdsl)
  pd_list <- PD_Assess(pd_input, cMethod = 'wilcoxon', bDataList = TRUE)
  expect_true('PValue' %in% names(pd_list$dfAnalyzed))
  pd_list <- PD_Assess(pd_input, cMethod = 'poisson', bDataList = TRUE)
  pd_list$dfAnalyzed
